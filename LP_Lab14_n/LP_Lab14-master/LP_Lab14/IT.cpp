...
int IT::IsId(IdTable& idtable, const char scope[SCOPE_MAXSIZE], const char id[ID_MAXSIZE]) {
	for (int i = 0; i < idtable.size; ++i) {
		if (idtable.table[i].idtype != IT::IDTYPE::L && !memcmp(scope, idtable.table[i].scope, (strlen(idtable.table[i].scope) < SCOPE_MAXSIZE ? strlen(idtable.table[i].scope) : SCOPE_MAXSIZE)) && !memcmp(id, idtable.table[i].id, (strlen(idtable.table[i].id) < ID_MAXSIZE ? strlen(idtable.table[i].id) : ID_MAXSIZE))) 
		{ return i; }
	}
